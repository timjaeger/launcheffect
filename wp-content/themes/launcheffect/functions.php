...
if (isset($_GET['activated']) && is_admin() && current_user_can('edit_posts')){
	...
	if(lefx_version() == 'premium'){
		...
		$documentationpage_check = get_page_by_title('Setup Instructions for v2.2', 'object', 'post');
		$documentationpage_content = <<<EOT
	
Welcome to Launch Effect Premium v2.2!  Launch Effect Premium lets you create and customize an entire website at the click of a few buttons.  Version 2.2 brings the theme to you with responsive capabilities and optimized mobile display.  Take a look around to see what's new and launch something today!
...
<h4>Step 2 &mdash; Set Image Sizes</h4>
...
<h4>Step 3 &mdash; Create Nav Menu</h4>
...
<h4>Step 4 &mdash; Select Widgets</h4>
...
<h4>Step 5 &mdash; Start Designing!</h4>
...
EOT;
		...
		$documentationpage_create = array(
			'post_type' => 'post',
			'post_title' => 'Setup Instructions for v2.2',
		...
		$documentationpage_check = get_page_by_title('Setup Instructions for v2.2 Lite', 'object', 'post');
		$documentationpage_content = <<<EOT

	
Welcome to Launch Effect v2.191 Lite!  Launch Effect Lite lets you create and customize a viral landing page at the click of a few buttons.  Version 2.2 brings the theme to you with responsive capabilities and optimized mobile display.  Take a look around to see what's new and launch something today!
...
EOT;
		...
		$documentationpage_create = array(
			'post_type' => 'post',
			'post_title' => 'Setup Instructions for v2.2 Lite',
			'post_content' => $documentationpage_content,
			'post_status' => 'publish',
			'post_author' => 1,
		);
...
function le_scripts() {
	...
	wp_register_script('le_script_supersize', get_template_directory_uri() . '/js/supersized.core.3.2.1.min.js', array('jquery'), '1.0' );
	...
	wp_register_script('lefx_script_spinjs', get_template_directory_uri() . '/js/spin.js', array('jquery'), '1.0' );
	wp_enqueue_script('lefx_script_spinjs');

	wp_register_script('lefx_script_imagesloaded', get_template_directory_uri() . '/js/jquery.imagesloaded.min.js', array('jquery'), '1.0' );
	wp_enqueue_script('lefx_script_imagesloaded');	
	
	//wp_register_script('lefx_script_fittext', get_template_directory_uri() . '/js/jquery.fittext.js', array('jquery'), '1.0' );
	//wp_enqueue_script('lefx_script_fittext');	
