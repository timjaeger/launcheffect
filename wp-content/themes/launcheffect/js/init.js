	...
	if($('#main').length) {
		if($('#hero').length) {
			$('#wrapper header img, #hero img').imagesLoaded(function(){
				$('#wrapper header, #hero, #sidebar').animate({opacity:1},300, function(){
					$('#main').animate({opacity:1},300, function(){
						$('#main').imagesLoaded( function($imagesToFadeIn) {
			...
			$('#wrapper header img').imagesLoaded(function(){
				$('#wrapper header, #sidebar').animate({opacity:1},300, function(){
					$('#main').animate({opacity:1},300, function(){
						$('#main').imagesLoaded( function($imagesToFadeIn) {
						  $imagesToFadeIn.animate({opacity:1},600);
						});
...
$(window).load(function(){
	...
	//$('header h1').fitText(1.2, { minFontSize: '20px', maxFontSize: '90px' });
...
$(document).ready(function(){
	...
	// EMBED WRAPPER
	$('iframe, object, embed').wrap('<div class="feature video" />');
	
	
	// RESPONSIVE NAV ARROW
	$('<span>&rsaquo;</span>').appendTo($('#nav-responsive nav').find('a'));
	$('#nav-responsive-menu-link').click(function(){
		$('#nav-responsive-menu-items nav').toggle();
		$('#nav-responsive-menu-items #launchlite').toggle();
		$('#nav-responsive-menu-link').toggleClass('open');
	});
        	...
        	function(data) {
				...
				function leSubmit(returning){
			    	...
			    	$('#success').fadeIn(function(){
						...
						$.scrollTo({top:successScroll+'px', left:'0px'}, 300);
			    	});
